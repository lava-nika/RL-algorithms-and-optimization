...
class ActorCriticAgent:
    ...
    def update(self, s_t, a_t, r_tp1, s_tp1, done):
        gamma = self.gamma
        alpha_theta = self.alpha_theta
        alpha_v = self.alpha_v

        v_s = self.V[s_t]
        v_next = 0.0 if done else self.V[s_tp1]
        delta = r_tp1 + gamma * v_next - v_s

        # Critic update
        self.V[s_t] += alpha_v * delta

        # Actor update
        probs = self.policy(s_t)
        grad_log_pi = -probs
        grad_log_pi[a_t] += 1.0
        self.theta[s_t] += alpha_theta * delta * grad_log_pi


def run_single_actor_critic(
...
def run_multiple_actor_critic(
    ...
    for run in range(n_runs):
        ...
        agent, returns, steps = run_single_actor_critic(
            env_name=env_name,
            map_name=map_name,
...
def plot_return_and_steps_smooth(
    ...
    plt.ylabel("Return")
    plt.title(
        f"One-Step Actor-Critic :Return vs Episode {title_suffix}\n"
    )
    plt.tight_layout()
    ...
    plt.title(
        f"One-Step Actor-Critic Algorithm: Steps vs Episode {title_suffix}\n"
    )
    plt.tight_layout()
    if save_prefix is not None:
...
if __name__ == "__main__":
    ...
    agents, mean_R, std_R, mean_S, std_S, all_R, all_S = run_multiple_actor_critic(
        n_runs=HP["n_runs"],
    ...
    avg_return_last100 = np.mean(mean_R[-100:])
    avg_steps_last100 = np.mean(mean_S[-100:])

    print(f"Average return over ALL episodes:       {avg_return_all:.4f}")
    print(f"Average steps over ALL episodes:        {avg_steps_all:.4f}")
    print()
    print(f"Average return over LAST 100 episodes:  {avg_return_last100:.4f}")
    print(f"Average steps over LAST 100 episodes:   {avg_steps_last100:.4f}")
    print("------------------------------------------------\n")
    ...
    plot_return_and_steps_smooth(
        ...
        save_prefix="actor_critic_frozenlake",
        show=True,
        smooth_window=200, 
    )
    ...
    print("Table summary for One-Step Actor Critic:")
